...
数据落盘根目录：`<workspaceDir>/.ocms/`（ocms 面向 Agent 本体，不同 agent 记忆隔离）。

`workspaceDir` 由 OpenClaw 的 `api.runtime.agent.resolveAgentWorkspaceDir(config, agentId)` 返回（即 agent 的 workspace，如 `~/.openclaw/workspace-<agent-id>/`），ocms 不硬编码具体路径，一律以该函数返回值为准。
...
认知 = Agent「世界是怎么运作的」知识/方法论。自由文本，存 `<workspaceDir>/.ocms/cognition/*.md`，由主 agent 惰性生成/维护，OpenClaw `memory_search` 检索。

### 3.3 品味（Taste）→ Markdown

品味 = 用户/Agent 的审美、偏好、判断倾向。存 `<workspaceDir>/.ocms/taste/*.md`，同样 `memory_search` 检索。
...
│   ├── store/
│   │   ├── paths.ts          # 数据路径解析（<workspaceDir>/.ocms/）
...
- [x] 数据落盘路径：`<workspaceDir>/.ocms/`（workspaceDir 由 resolveAgentWorkspaceDir 返回）
